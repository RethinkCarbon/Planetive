...
import { WHAT_WE_DO_NAV } from "@/lib/what-we-do-content";

type NavWhatWeDoMenuProps = {
  isSolid: boolean;
  onNavigate?: () => void;
  variant?: "dropdown" | "list";
};

export function NavWhatWeDoMenu({
  isSolid,
  onNavigate,
  variant = "dropdown",
}: NavWhatWeDoMenuProps) {
        ...
        return (
          <li key={item.to}>
            <Link
              ...
              role="menuitem"
              onClick={close}
              className={itemClass(active, index, WHAT_WE_DO_NAV.length)}
            >
              <span
                className="h-2 w-2 shrink-0 rounded-full"
                style={{ backgroundColor: item.colorActive }}
                aria-hidden
              />
              {item.label}
            </Link>
